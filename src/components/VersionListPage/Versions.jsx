...
class Versions extends React.Component {
    ...
    renderList = () => {
        const { networkError, name, createdAt, versions } = this.props.currentRecipe
        if (!networkError) {
            if (versions) {
                return (
                    <div className="versionsWrapper">
                        <div className="versionsHeader"><span style={{ fontSize: '20px', fontWeight: '700', margin: '3px' }}>{name}</span><span style={{ opacity: '0.8', fontStyle: 'Italic', margin: '3px' }}>{this.showCreationDate(createdAt)}</span></div>
                            {versions ? (
                            versions.reverse().map(version => (
                                <div className="recipeVersionWrapper">{version.description}</div>
                            ))
                             ) : null}
                             <div>
