...
const ModalWindow = ({ modalWindowType, callModalWindow, handleSubmit, pristine, reset, submitting }) => {
  ...
  return (
    <div>
      ...
      >
        ...
        <div style={{ width: '100%', textAlign: 'right' }}><button style={{ width: '30px', height: '30px', borderRadius: '50%', border: 'none', outline: 'none', lineHeight: '20px', fontSize: '20px', margin: '5px 5px 0 0', cursor: 'pointer', textAlign: 'center' }} onClick={closeWindow}>&times;</button></div>
  ...
  <div className="modalWindowButtons"><Button disabled={pristine || submitting} variant="contained" style={{ width: '120px', margin: '0 5px' }} type='button' onClick={reset}>Clear</Button><Button disabled={pristine || submitting} variant="contained" color="primary" style={{ width: '120px', margin: '0 5px' }} type='submit'>{modalWindowType}</Button></div>
  </form>
        </div>
      </Modal>
    </div>
